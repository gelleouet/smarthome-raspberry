 ...
 */
var DEFAULT_VALUES = ["level", "sensor"]
	...
	this.zwave.on('value changed', function(nodeid, comclass, value) {
		...
		if (device.networkScan && oldValue.value != value.value) {
			if (device.isMetadata(value)) {
				...
				device.sendDeviceAloneValue(value)
			} else {
 ...
 * principal
 */
ZWave.prototype.sendDeviceAloneValue = function(metaName) {
	if (metaName.value) {
		var device = new ZWave(this.server)
		device.mac = "zwave" + metaName.value_id
		device.label = metaName.label
		device.value = metaName.value
